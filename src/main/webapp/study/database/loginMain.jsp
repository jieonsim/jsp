...
<div class="container">
  ...
  <div class="text-right mb-3">
  	<c:if test="${pag > 1}">
		  	<a href="${ctp}/study/database/LoginList?pag=1" title="첫 페이지">◁</a>
		  	<a href="${ctp}/study/database/LoginList?pag=${pag-1}" title="이전 페이지">←</a>
		  	</c:if>
		  	${pag}/${totPage}
		  	<c:if test="${pag < totPage}">
		  	<a href="${ctp}/study/database/LoginList?pag=${pag+1}" title="다음 페이지">→</a>
		  	<a href="${ctp}/study/database/LoginList?pag=${totPage}" title="마지막 페이지">▷</a>
  	</c:if>
  </div>
  <div class="row">
