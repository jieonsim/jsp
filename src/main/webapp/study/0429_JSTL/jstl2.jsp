...
<div class="container">
  ...
  <hr>
  <h3>JSTL 반복문(core 라이브러리 사용 : foreach 문)</h3>
  <pre>
  	사용법1 : < c : forEach var="변수" begin="초기값" end="최종값" step="증감값" varStatus="매개변수"> < / c : forEach >
  	: step이 1인 경우 생략 가능하다. varStatus="변수" 는 필요할 때만 선언해서 사용하면 된다.
  ...
  <p>사용법 1 </p>
  <%-- <%for(int i = 1; i <= 10; i++) {} %> --%>
  <%-- <c:forEach var="i" begin="1" end="10" step="1" varStatus="st"> step , varStatus 생략 가능--%>
  ...
  <p>사용법2 </p>
  <%
  	String[] cards = {"국민", "BC", "현대", "삼성", "농협", "비자"};
  ...
  <hr>
  <p>사용법3 </p>
  <c:set var="hobbies" value="등산/낚시/수영/바둑/바이크/독서/영화감상" />
  취미 : ${hobbies}<br>
  취미를 콤마(,)로 분리 후 출력 : 
  ...
  <p>사용법4(상태변수 사용예제)</p>
  <pre>
  	상태변수 예약어 : count - 1부터 1씩 증가 처리,
  				  index - 인덱스번호(0번부터 시작) 
  				  current - 현재 아이템 출력(var값이랑 똑같이 나옴)
  				  first - 첫번째 아이템인지 물어봄
  				  last - 마지막번째 아이템인지 물어봄
  				  위 예약어를 쓰기 위해서는 varStatus="st"를 써줘야함
  </pre>
  ...
  * 5단 * <br>
  ...
  <div>구구단 3단~5단까지 출력하세요(2중 for문)</div>
  ...
  <hr>
	...
	<h5>4. 배열에 그림명을 저장 후 배열에 저장된 그림을 출력하시오.(단 1줄에는 3개의 그림씩 출력하시오)</h5>
		<div>
		<c:set var="images" value="111.jpg/112.jpg/113.jpg/114.jpg/115.jpg"></c:set>
	...
	<h5>7. 나이가 25세 미만은 굵고 파랑색으로 표시</h5>
	  <c:forEach var="member" items="${members}">
	  	 <c:forEach var="m" items="${member}">
	  ...
	  <hr>
	  <h5>9. 앞의 사용법 2번에 저장한 cards의 내용 중, '국민카드'는 파랑색, '삼성카드'는 빨강색, 첫번째 카드의 배경색은 노랑, 마지막 카드의 배경색은 하늘색으로 출력하시오</h5>
</div>
<p><br/></p>
