		...
		<div class="text-center">
			<table class="table table-bordered" style="height: 450px">
				...
				<tr>
					...
					<c:forEach begin="${preLastDay - (startWeek - 2)}" end="${preLastDay - 1}">
						<td style="font-size: 0.7em; color:gray; text-align: left;">${preYear}-${preLastMonth + 1}-${preLastDay}</td>
					</c:forEach>

					<!-- 해당월의 1일을 startWeek위치부터 출력한다.(날짜는 1씩 증가시켜주고, 7칸이 될때 줄바꿈 처리한다.) -->
					<c:set var="cell" value="${startWeek}"></c:set>
					...
					<c:set var="todaySw" value="${toYear==yy && toMonth==mm && toDay==st.count ? 1 : 0}"></c:set>
						...
						<c:set var="cell" value="${cell + 1}"></c:set>
					</c:forEach>
					
					<!-- 마지막일 이후를 다음달의 시작일자로부터 채워준다. -->
					<c:if test="${(cell - 1) % 7 != 0}">
						<c:forEach begin="${nextStartWeek}" end="7" varStatus="st">
							<td style="font-size: 0.7em; color:gray; text-align: left;">${nextYear}-${nextMonth + 1}-${st.count}</td>
						</c:forEach>
					</c:if>
				</tr>
			</table>
