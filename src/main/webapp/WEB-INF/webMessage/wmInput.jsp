		...
		<form name="myform" method="post" action="WmInputOk.wm">
			<table class="table table-bordered">
				<tr>
					<th>보내는 사람</th>
					<td>
						<input type="text" name="sendId" value="${sMid}" readonly class="form-control" />
					</td>
				</tr>
				<tr>
					<th>받는 사람</th>
					<td>
						<!-- 주소록버튼을 클릭해서 아이디를 선택하면 받는사람 입력란에 표시되도록 작성 -->
						<c:if test="${empty param.receiveId}">
							<div class="input-group">
								<input type="text" name="receiveId" placeholder="받는사람 아이디" class="form-control" />
								<div class="input-group-append">
									<a href="#" id="jusorokBtn" class="btn btn-info" data-toggle="modal" data-target="#myModal">주소록</a>
								</div>
							</div>
						</c:if>
						<c:if test="${!empty param.receiveId}">
							<input type="text" name="receiveId" value="${param.receiveId}" readonly class="form-control" />
						</c:if>
					</td>
				</tr>
				<tr>
				...
				<div class="m-3">
					<div class="row">
						...
						<div class="col text-right">
							<input type="button" value="아이디검색" onclick="idSearchView()" id="idSearchShowBtn" class="btn btn-success btn-sm text-right" />
						</div>
					</div>
					...
					<table class="table table-hover">
						...
						<c:forEach var="vo" items="${mVos}" varStatus="st">
							<tr>
								<td>${st.count}</td>
								<td>
									<a href="WebMessage.wm?mSw=0&receiveId=${vo.mid}">${vo.mid}</a>
								</td>
								<td>${vo.nickName}</td>
							</tr>
						</c:forEach>
