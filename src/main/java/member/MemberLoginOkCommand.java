...
public class MemberLoginOkCommand implements MemberInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2-2.준회원을 자동으로 정회원 등업 처리
		// 3. 처리완료된 자료(vo)를 DB에 업데이트 해준다.
		...
		// strToday와 lastdate가 같지 않다면 오늘 접속하지 않았다는 것
		if(!strToday.equals(vo.getLastDate().substring(0, 10))) {
			// 오늘 처음 방문한 경우. (오늘 방문 카운트는 1로, 기존 포인트에 +10)
			vo.setTodayCnt(1);
			vo.setPoint((vo.getPoint() + 10));
		}
		else {
			// 오늘 다시 방문한 경우 (오늘 방문 카운트는 오늘 방문 카운트 + 1, 포인트 증가는 오늘 방문 횟수가 5회 전까지라면 기존 포인트에 + 10을 한다)
